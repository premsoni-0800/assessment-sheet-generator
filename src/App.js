...
export default function App() {
  ...
  const handleSplashDone = () => {
    setSplashDone(true);
    // tiny delay so React paints the app div before we unblur it
    setTimeout(() => setAppReady(true), 60);
  };
  // ────────────────────────────────────────────────────────────────────────
  ...
  return (
    <>
      {/* ── Splash screen — sits on top until dismissed ────────────────── */}
      {!splashDone && <SplashScreen onDone={handleSplashDone} />}

      {/* ── Main app — rendered underneath, unblurs in as splash leaves ── */}
      <div style={{
        filter:    appReady ? 'blur(0) brightness(1)'        : 'blur(18px) brightness(0.6)',
        opacity:   appReady ? 1                              : 0,
        transform: appReady ? 'scale(1)'                    : 'scale(1.03)',
        transition: 'filter 1.1s cubic-bezier(0.16,1,0.3,1), opacity 1.1s ease, transform 1.1s cubic-bezier(0.16,1,0.3,1)',
